...
class InsuranceHybridSearchIndex:
    ...
    def add_chunks(self, chunks: List[Chunk], display_name: str = ""):
        ...
        # Rebuild TF-IDF on full corpus (re-fit needed for new vocab)
        all_texts    = [c.text for c in self._chunks]
        self.embedder.fit(all_texts)
        self._vectors = self.embedder.encode(all_texts, normalize_embeddings=True)

        # Recreate ChromaDB collection if dim changed or first use
        ...
        if need_recreate:
            if stored_dim > 0 and stored_dim != new_dim:
                console.print(f"[yellow]  Embedder dim changed {stored_dim}→{new_dim}[/]")
            try: self._chroma.delete_collection(COLLECTION_NAME)
            except Exception: pass
            self._col = self._chroma.create_collection(
                COLLECTION_NAME, metadata={"hnsw:space":"cosine","dim":str(new_dim)})
            # Save ALL current chunks (re-snapshot entire corpus)
            all_display = {c.chunk_id: self._name_map.get(c.chunk_id, c.doc_name)
                           for c in self._chunks}
            all_display.update({c.chunk_id: self._name_map.get(c.chunk_id, c.doc_name)
                                 for c in self._parent_map.values()})
            all_chunks_flat = list(self._chunks) + list(self._parent_map.values())
            self._save_to_chroma(all_chunks_flat, self._vectors, all_display)
        else:
            # Incremental: only save newly added chunks
            new_vecs = self._vectors[-len(child_chunks):]
            all_new  = list(chunks)  # includes parent + child of this batch
            self._save_to_chroma(all_new, new_vecs, display_names)
